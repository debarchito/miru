...
type history =
  {mutable entries: string list; mutable index: int; mutable saved: string}
...
let save_history h =
  try
    ...
    List.iter
      (fun l -> output_string ch l ; output_char ch '\n')
      (List.rev h.entries) ;
    close_out ch
  with _ -> ()
...
let read_line ~prompt ~prompt_len history =
  ...
  let rec loop () =
    ...
    | Ctrl 'C' ->
        raise Exit
    | Ctrl 'D' ->
        if Buffer.length buf = 0 then raise End_of_file else loop ()
    | _ ->
        loop ()
  in
  match loop () with
...
let run () =
  ...
  if not (Unix.isatty Unix.stdin) then begin
    ...
    save_history history ; leave_raw ()
  end

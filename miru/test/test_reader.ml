...
let rec equal_form a b =
  ...
  | F.Float a, F.Float b ->
      a = b
  | F.String a, F.String b ->
      a = b
  | F.Symbol a, F.Symbol b ->
      a = b
  | F.List a, F.List b ->
      equal_lists a b
  ...
  | F.Field a, F.Field b ->
      equal_form a b
  | F.MutableField a, F.MutableField b ->
      a = b
  | F.Type a, F.Type b ->
      equal_form a b
  | F.TypeApplication (s1, a1), F.TypeApplication (s2, a2) ->
      s1 = s2 && equal_form a1 a2
  | F.Tag (a1, a2), F.Tag (b1, b2) ->
      a1 = b1 && equal_form a2 b2
  | F.Fn a, F.Fn b ->
      equal_lists a b
  | F.Quote a, F.Quote b ->
      equal_form a b
  | F.Quasiquote a, F.Quasiquote b ->
      equal_form a b
  | F.Unquote a, F.Unquote b ->
      equal_form a b
  | F.Splice a, F.Splice b ->
      equal_form a b
  | F.Record (n1, b1), F.Record (n2, b2) ->
      n1 = n2 && equal_form b1 b2
  | F.AbstractType (n1, b1), F.AbstractType (n2, b2) ->
      n1 = n2 && equal_form b1 b2
  | F.Constructor (n1, p1), F.Constructor (n2, p2) ->
      n1 = n2 && p1 = p2
  | F.Variant (n1, c1), F.Variant (n2, c2) ->
      n1 = n2 && equal_lists c1 c2
  | _ ->
      false
...
let test_struct_basic () =
  ...
  check_form "basic struct"
    [ F.RecordExpression
        [ (F.Field (F.Symbol "key1"), F.String "value")
        ; (F.Field (F.Symbol "key2"), F.String "value2") ] ]
    forms
